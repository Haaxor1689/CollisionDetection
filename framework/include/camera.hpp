...
private:
    ...
    ///		- Sensitivity of the mouse when changing elevation or direction
    ///angles
    ...
    /// True or false if moused buttons are pressed and the user rotates/zooms the
    /// camera
    bool is_rotating, is_zooming;

    /// Recomputes 'eye_position' from 'angle_direction', 'angle_elevation', and
    /// 'distance'
    void update_eye_pos();
...
public:
    ...
    /// Call when the user presses or releases a mouse button (see
    /// glfwSetMouseButtonCallback)
    void on_mouse_button(int button, int action, int mods);

    /// Call when the user moves with the mouse cursor (see
