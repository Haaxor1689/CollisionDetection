...
TEST_CASE("Ball-Brick collision detection") {
    BallCollider ball1{ Vector(3.f, 0.f, 0.f), Vector(0.f), 1.f };

    CHECK_FALSE(ball1.DidCollide(BrickCollider{ 0.5f, segments / 4 }));
    CHECK_FALSE(ball1.DidCollide(BrickCollider{ 4.5f, segments / 4 }));
    CHECK(ball1.DidCollide(BrickCollider{ 1.9f, segments / 4 }));
    CHECK(ball1.DidCollide(BrickCollider{ 3.3f, segments / 4 }));

    BallCollider ball2{ Vector(3.f, 0.f, 1.1f), Vector(0.f), 1.f };
    ...
    CHECK(BallCollider{ Vector(2.9f, 0.f, -1.1f), Vector(0.f), 1.f }.DidCollide(brick1));
    CHECK_FALSE(BallCollider{ Vector(-1.1f, 0.f, -2.9f), Vector(0.f), 1.f }.DidCollide(brick1));
    CHECK_FALSE(BallCollider{ Vector(-1.1f, 0.f, 2.9f), Vector(0.f), 1.f }.DidCollide(brick1));
}